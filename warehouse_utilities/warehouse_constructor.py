...
class WarehouseConstructor:
    ...
    def calculate_num_of_shelves(self):
        ...
        """
        shelf = Shelves()
        num_shelves_back_wall = (self.width // shelf.length) * (
            self.height // shelf.height
        )
        x_shelves = (self.width - self.lane_width_size) // (shelf.length)
        y_shelves = (self.length - shelf.depth - self.lane_width_size) // (
            shelf.depth + self.lane_width_size
        )
        ...
        (
            ...
            self.y_grid_space,
            self.z_grid_space,
            self.leftover_x_space,
        ) = (x_shelves, y_shelves, z_shelves, self.width - (x_shelves * shelf.length))
        return x_shelves, y_shelves, z_shelves, num_shelves_back_wall

    def place_shelves(self):
        self.calculate_num_of_shelves()
        self.grid = np.zeros(
            (self.x_grid_space, self.y_grid_space + 1, self.z_grid_space), "U32"
        )
